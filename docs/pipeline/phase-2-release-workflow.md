...
Phase 1에서 정리된 아이템 중, 이번에 배포 대상으로 검토할 후보를 모은다.

### 현재 상태

사람이 Eagle 최근 생성/수정 아이템을 우선 살펴보며 배포 후보를 추린다.

### 목표 상태

최근 변경 아이템과 이미 배포된 범위를 비교해, 배포 후보 큐를 안정적으로 만들 수 있다.
...
이번에 실제로 반영할 아이템 집합을 명시적으로 고정한다.

### 현재 상태

작은 배치부터 검증하는 운영 원칙이 확정되어 있고, 현재는 10개 수준의 MVP 배치를 기준으로 작업 중이다.

### 목표 상태

배치 정의 파일 하나만 보면 이번 배포 범위를 정확히 알 수 있게 만든다.
...
- 이번에 올릴 아이템 ID 집합을 정한다.
- 검증 목적의 작은 배치인지, 확대 배치인지 구분한다.
- 배치 범위를 문서나 설정으로 명시한다.
...
- 검증용 10개와 실제 배포 범위가 섞이는 경우
- 배치에 넣을지 말지 애매한 아이템이 남는 경우
- 명시된 배치 없이 사람이 기억으로만 운영하는 경우
...
- 향후 기준 설정 파일: `config/release-batch.json`
...
`scripts/export.mjs`가 clip JSON, index, video, preview, thumbnail을 생성한다.

### 목표 상태

현재 배치만 대상으로 안전하게 생성하고, 생성 결과가 항상 같은 규약을 따른다.
...
- 같은 배치의 clip이 browse/detail/search에서 일관되게 보인다.
...
- [Search 페이지](/Users/macbook/reflix-nextjs/src/app/[lang]/search/page.tsx)
