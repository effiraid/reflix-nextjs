...
현재 운영의 기준 파일은 [`config/release-batch.json`](/Users/macbook/reflix-nextjs/config/release-batch.json)과 [`config/published-state.json`](/Users/macbook/reflix-nextjs/config/published-state.json)이다. 운영 흐름은 `release:scan -> release:review -> Eagle review -> release:approve -> export:batch:dry -> export:batch -> release:mark-published / release:mark-failed` 순서로 진행한다. 기본 `release:scan`은 active batch 범위만 스캔하고, 전체 eligible 라이브러리 스캔은 명시적인 `release:scan:all`로만 연다.
