...
```
/[lang]/ ..................... 홈 (히어로 + 카테고리 + 인기 클립)
/[lang]/browse .............. 3패널 탐색 (핵심)
/[lang]/browse?category=combat  카테고리 필터 적용
/[lang]/browse?tag=화남&tag=검  다중 태그 필터 적용
/[lang]/clip/[id] ........... 클립 상세 (비디오 플레이어 + 메타데이터)
/[lang]/search .............. 검색 결과
...
| 페이지 | 렌더링 | 이유 |
|---|---|---|
| `/[lang]/` (홈) | SSG | 정적 콘텐츠, 빌드 시 생성 |
| `/[lang]/browse` | SSG | index.json 클라이언트 필터링, 서버 로직 없음 |
| `/[lang]/clip/[id]` | ISR | 7,000~100,000개 → 빌드 시 전부 생성 불가. 첫 접속 시 생성 후 캐싱 |
| `/[lang]/search` | SSG | Pagefind 클라이언트 검색, 서버 로직 없음 |
...
├── ko.json                 ← 한국어 UI 문자열
├── en.json                 ← 영어 UI 문자열
└── index.ts                ← getDictionary() 로더 (Next.js 16 공식 패턴)
...
Eagle Library (/images/{id}.info/)
    ...
    │     - LQIP 생성 (32px 첫 프레임 JPEG → base64 인코딩 → JSON에 삽입, 파일 삭제)
    │
