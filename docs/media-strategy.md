...
- 대신 browse/search는 discovery surface로 취급하되, 목록 자체는 guest/free/pro 모두 전체 아이템을 노출한다.
- guest는 browse/search에서 모든 카드를 블러 처리하고 로그인 전에는 어떤 카드도 열 수 없다.
- free는 browse/search에서 전체 카드를 보되 처음 5개만 잠금 해제하고, 나머지는 블러 + 잠금 상태로 유지한다.
- pro는 browse/search에서 전체 카드를 잠금 없이 연다.
- 요약하면: `발견 목록은 전체 노출`, `열람 권한은 tier별 잠금`, `공유된 상세 링크 소비 흐름은 개방`이 기본 원칙이다.
